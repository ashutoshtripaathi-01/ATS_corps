...
export default function CandidateDB() {
  ...
  useEffect(() => {
    getAdminCandidates()
      .then((data) =>
        setCandidates(
          data.filter((c: Candidate) => c.verification_status === 'verified'),
        ),
      )
      .catch(console.error)
  ...
  const forces = [
    'all',
    ...Array.from(new Set(candidates.map((c) => c.force))),
  ];
    ...
    <div className='p-4 sm:p-6 max-w-3xl mx-auto'>
      <div className='mb-5'>
        ...
        <p className='text-sm text-gray-500'>
          {loading
            ? 'Loading…'
            : `${filtered.length} verified ex-servicemen available`}
        </p>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        ...
        >
          ...
          <p className='text-xs text-gray-400 max-w-xs mx-auto'>
            Once Ex-Serviceman Jobs verifies candidate registrations, they will
            appear in this database for you to browse.
          </p>
        </motion.div>
      )}
          ...
          {filtered.map((c, i) => (
            ...
            >
              <div className='flex items-start gap-3 mb-3'>
                ...
                <span
                  className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2.5 py-1 rounded-full shrink-0 ${FORCE_COLOR[c.force] ?? 'bg-gray-100 text-gray-600'}`}
                >
