...
export default function EmployerSettings() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-lg mx-auto">
      <div className="mb-5">
        <h1 className="text-xl font-extrabold text-gray-900 mb-0.5" style={{ fontFamily: 'Plus Jakarta Sans' }}>Settings</h1>
      ...
      >
        ...
        <div className="divide-y divide-gray-50">
          {[
            { key: 'newApplications',   label: 'New Applications',    desc: 'Alert when a candidate applies' },
            { key: 'candidateMessages', label: 'Candidate Messages',  desc: 'New messages from candidates' },
            { key: 'systemAlerts',      label: 'System Alerts',       desc: 'Platform updates and announcements' },
          ].map(({ key, label, desc }) => (
            <div key={key} className="flex items-center justify-between px-4 py-3.5 gap-3">
              ...
              >
                <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all ${notifications[key as keyof typeof notifications] ? 'left-5' : 'left-0.5'}`} />
              </button>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <div className="divide-y divide-gray-50">
          {['Privacy Policy', 'Terms of Service', 'Refund Policy'].map((label) => (
            <button key={label} className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-gray-50 transition-colors text-left">
      ...
      <p className="text-center text-xs text-gray-400 mt-4">ATS Corps v1.0 · Recruiter Portal</p>
    </div>
  )
}
