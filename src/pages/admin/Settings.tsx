...
import { Settings, Shield, Bell, LogOut, ChevronRight, Key, Database } from 'lucide-react'
...
export default function AdminSettings() {
  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      toast({ title: data.status === 'ok' ? 'API is healthy' : 'API issue detected', variant: data.status === 'ok' ? 'success' : 'error' })
    } catch {
      setDbStatus('error')
      toast({ title: 'Cannot reach API server', description: 'Make sure the backend is running on port 4000.', variant: 'error' })
    } finally {
      setChecking(false)
    }
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-lg mx-auto">
      <div className="mb-5">
        <h1 className="text-xl font-extrabold text-gray-900 mb-0.5" style={{ fontFamily: 'Plus Jakarta Sans' }}>Admin Settings</h1>
        <p className="text-sm text-gray-500">System configuration and account management</p>
      </div>
      ...
      >
        ...
        <div className="divide-y divide-gray-50">
          <div className="flex items-center justify-between px-4 py-3.5">
            <div>
              <p className="text-sm font-semibold text-gray-900">{user?.name ?? 'ATS Corps Admin'}</p>
              <p className="text-xs text-gray-400 mt-0.5">{user?.email ?? 'admin@atscorps.in'}</p>
            </div>
            <span className="text-[10px] font-bold bg-[#F7A607]/10 text-[#F7A607] px-2.5 py-1 rounded-full">Super Admin</span>
          </div>
          <div className="flex items-center justify-between px-4 py-3.5">
          ...
          <div className="flex items-center justify-between px-4 py-3.5">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {dbStatus === 'ok' ? '✅ API is running normally' : dbStatus === 'error' ? '❌ API unreachable' : 'Check if backend API is online'}
              </p>
            </div>
      ...
      <p className="text-center text-xs text-gray-400 mt-4">ATS Corps Admin Panel v1.0</p>
    </div>
  )
}
