...
import { ArrowLeft, CreditCard, MapPin, Shield, CheckCircle, User, Briefcase } from 'lucide-react'
...
export default function CandidatePayment() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      await new Promise<{ razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }>(
        (resolve, reject) => {
      ...
      <div className='min-h-screen bg-gray-50 flex flex-col items-center justify-center px-6'>
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            <p className='text-gray-500 text-sm mb-1'>₹1 received · ATS Corps Registration</p>
            <p className='text-gray-400 text-xs mb-8'>{state.fullName} · {state.loc1}</p>
      ...
      <div className='flex-1 px-4 py-6 max-w-sm mx-auto w-full space-y-4'>
        ...
        >
          ...
          <div className='flex items-baseline gap-1.5 mb-1'>
            <span className='text-3xl font-extrabold'>₹{state.applicationFee.toLocaleString()}</span>
          ...
          <div className='pt-3 border-t border-white/10 space-y-2'>
            {[
              ...
              <div key={label} className='flex justify-between items-center'>
                ...
                <span className='text-xs font-semibold text-white truncate max-w-[55%] text-right'>{value}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className='grid grid-cols-3 gap-2'>
            {['UPI', 'Cards', 'Net Banking'].map((m) => (
              <div key={m} className='flex flex-col items-center gap-1 py-2.5 px-2 bg-gray-50 rounded-xl'>
