...
import {
  User, Phone, Shield, Briefcase, MapPin, CalendarDays,
...
export default function CandidateProfile() {
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center shadow-sm">
          ...
          <p className="text-sm font-semibold text-gray-700 mb-1">Profile not found</p>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-4">
      <div className="mb-1">
        <h1 className="text-xl font-extrabold text-gray-900" style={{ fontFamily: 'Plus Jakarta Sans' }}>My Profile</h1>
        <p className="text-sm text-gray-500 mt-0.5">Your registration details on file with ATS Corps</p>
      </div>
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-lg font-extrabold leading-tight" style={{ fontFamily: 'Plus Jakarta Sans' }}>
              {profile.full_name}
            </p>
          ...
          <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full border ${verifyBadge.style} shrink-0`}>
      ...
      >
        ...
        <div className="p-4 grid grid-cols-2 gap-3">
          {[
            ...
            { label: 'Retirement Date', value: new Date(profile.retirement_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) },
          ].map(({ label, value }) => (
      ...
      >
        ...
        <div className="p-4 space-y-3">
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Preferred Post</p>
          ...
          {profile.gun_license && (
            <div>
              ...
              <p className="text-sm font-semibold text-gray-800">{profile.gun_license}</p>
            </div>
          )}
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Preferred Locations</p>
            <div className="flex flex-wrap gap-2">
              {[profile.loc1, profile.loc2, profile.loc3].filter(Boolean).map((loc) => (
                <span key={loc} className="flex items-center gap-1 text-xs font-medium bg-[#F7A607]/10 text-[#F7A607] px-2.5 py-1 rounded-full">
      ...
      >
        ...
        <div className="p-4 space-y-2.5">
          {docItems.map(({ label, path }) => (
            <div key={label} className="flex items-center justify-between gap-3 bg-gray-50 rounded-xl p-3">
      ...
      <p className="text-center text-xs text-gray-400 pb-2">
        ...
        <br />Contact ATS Corps admin to update your details.
      </p>
    </div>
  )
}
