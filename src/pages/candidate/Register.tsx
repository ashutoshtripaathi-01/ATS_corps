...
> = {
  ...
  'Air Force': {
    icon: Plane,
    color: 'text-sky-600',
    description: 'Indian Air Force',
  },
  'Para Military': {
    icon: Star,
    color: 'text-orange-600',
    description: 'CRPF / BSF / CISF / ITBP',
  },
};
...
function ServiceStep({
  ...
  return (
    <div className='space-y-4 px-4 pt-4 pb-6'>
          ...
          {(FORCES as unknown as Force[]).map((f) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon
                    className={`w-4 h-4 ${selected ? 'text-[#F7A607]' : meta.color}`}
                  />
                </div>
                <div className='min-w-0'>
                  <p
                    className={`text-sm font-semibold leading-tight ${selected ? 'text-[#292e31]' : 'text-gray-800'}`}
                  >
                    {f}
                  </p>
      ...
      <div>
        <FileUpload
          label='Ex-Serviceman Identity Card'
          required
          hint='Upload front side — clear photo or scanned copy'
          file={data.idCard}
    ...
    <div className='px-4 pt-4 pb-6 space-y-4'>
      <div className='flex gap-3 p-3.5 bg-blue-50 border border-blue-100 rounded-xl'>
        ...
        <p className='text-xs text-blue-700 leading-relaxed'>
          All documents are encrypted and stored securely. Used only for
      ...
      <div className='bg-white border border-gray-100 rounded-2xl p-4 space-y-3 shadow-sm'>
        ...
        <FileUpload
          ...
          required
          hint='Army / Navy / Air Force / Para Military Discharge Book'
      ...
      <div className='bg-white border border-gray-100 rounded-2xl p-4 space-y-3 shadow-sm'>
        <div className='flex items-center gap-2.5'>
          ...
          <div>
            ...
            <p className='text-xs text-gray-500'>
              Clearance certificate from local police
            </p>
          </div>
        </div>
        <FileUpload
    ...
    <div className='px-4 pt-4 pb-6 space-y-4'>
          ...
          {(POSTS as unknown as string[]).map((post) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${active ? 'text-[#292e31]' : 'text-gray-700'}`}
                >
                  {post}
                </span>
              </button>
      ...
      <div className='bg-white border border-gray-100 rounded-2xl p-4 shadow-sm space-y-4'>
        <div className='flex items-start gap-2'>
          ...
          <div>
            <p className='text-sm font-semibold text-gray-900'>
              Preferred Posting Location
            </p>
            <p className='text-xs text-gray-500 mt-0.5'>
              Arrange in order of preference. No location can repeat.
            </p>
          </div>
        </div>
        ...
        {[
          {
            ...
            label: 'Priority 1',
            badge: '1st Choice',
            badgeColor: 'bg-[#F7A607] text-white',
          },
          {
            key: 'loc2' as const,
            label: 'Priority 2',
          ...
          const excluding =
            key === 'loc1'
              ...
              : key === 'loc2'
                ...
                : [data.loc1, data.loc2];
          const isDisabled =
            (key === 'loc2' && !data.loc1) || (key === 'loc3' && !data.loc2);
          return (
            <div key={key}>
        ...
        {data.loc1 && (
          ...
          >
            ...
            <p className='text-lg font-extrabold text-[#F7A607]'>
              ₹{LOCATION_FEES[data.loc1]?.toLocaleString()}
            </p>
          </motion.div>
        )}
      </div>
...
function PaymentStep({
  ...
  if (!data.loc1) {
    return (
      <div className='px-4 pt-4 pb-6 flex flex-col items-center justify-center min-h-[50vh] text-center gap-4'>
  ...
  return (
    <div className='px-4 pt-4 pb-6 space-y-4'>
      ...
      {/* Proceed button → navigates to dedicated payment page */}
      <Button size='lg' className='w-full rounded-xl text-sm font-bold gap-2.5 h-13' onClick={onProceed}>
        <CreditCard className='w-4 h-4' />
        Proceed to Payment
      </Button>

      <p className='text-center text-xs text-gray-400 flex items-center justify-center gap-1.5 pb-1'>
...
/* ─── Step indicator ─────────────────────────────────────────────────── */
const STEPS = ['Service', 'Documents', 'Preference', 'Payment'];
  ...
  const validate = (): boolean => {
    ...
    if (step === 1) {
      ...
      if (!data.retirementDate)
        e.retirementDate = 'Date of retirement is required';
    }
    if (step === 2) {
    ...
    if (step === 3) {
      ...
      if (data.post === 'Other' && !data.otherPost.trim())
        e.otherPost = 'Please specify the post';
    ...
    <div className='min-h-screen bg-gray-50 flex flex-col max-w-lg mx-auto'>
      ...
      <div className='sticky top-0 z-40 bg-white border-b border-gray-100'>
        <div className='flex items-center gap-3 px-4 h-14'>
          ...
          <span className='text-xs font-semibold text-[#F7A607] shrink-0'>
            {step}/4
          </span>
        </div>
        ...
        <div className='flex border-b border-gray-100'>
          {STEPS.map((label, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`text-[11px] font-semibold ${active ? 'text-[#F7A607]' : done ? 'text-green-600' : 'text-gray-400'}`}
                >
                  {done ? '✓' : s}
                </span>
                <span
                  className={`text-[9px] font-medium ${active ? 'text-[#F7A607]' : done ? 'text-green-600' : 'text-gray-400'}`}
                >
                  {label}
                </span>
              </div>
      ...
      <div className='flex-1 overflow-y-auto pb-32'>
        <AnimatePresence mode='wait'>
          ...
          >
            {step === 1 && (
              <ServiceStep data={data} update={update} errors={errors} />
            )}
            {step === 2 && (
              <DocumentsStep data={data} update={update} errors={errors} />
            )}
            {step === 3 && (
              <PostLocationStep data={data} update={update} errors={errors} />
            )}
            {step === 4 && (
              <PaymentStep
                data={data}
                onProceed={() => {
                  navigate('/candidate/payment', {
                    state: {
                      ...
                      idCard:          data.idCard,
                      dischargeBook:   data.dischargeBook,
      ...
      {/* ── Fixed bottom bar (steps 1–3 only) ── */}
      {step < 4 && (
