...
const card =
  'bg-white dark:bg-[#1e2227] border border-gray-100 dark:border-white/8 rounded-2xl shadow-sm';
const cardHeader =
  'flex items-center justify-between px-4 py-3 border-b border-gray-50 dark:border-white/8';
...
export default function CandidateDashboard() {
  ...
  const docCount = profile
    ? [
        ...
        profile.discharge_book_path,
        profile.police_verification_path,
    ...
    <div className='p-4 sm:p-6 max-w-4xl mx-auto space-y-5'>
      ...
      {!loading && error && (
        <div className='flex items-center gap-3 p-4 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-2xl'>
          ...
          <p className='text-sm text-red-700 dark:text-red-400'>
            Could not load your profile. Please check your connection or contact
            support.
          </p>
        </div>
      )}
            ...
            {[
              {
                ...
                sub:
                  profile.payment_status === 'paid'
                    ? `₹${profile.application_fee.toLocaleString()} paid`
                    : 'Action required',
              },
              {
                label: 'Verification',
              ...
              },
              {
                ...
                sub:
                  docCount === 3 ? 'All submitted' : `${3 - docCount} pending`,
              },
              ...
              >
                <div
                  className={`w-9 h-9 rounded-xl ${bg} flex items-center justify-center mb-0 sm:mb-3 shrink-0`}
                >
                  <Icon className={`w-4 h-4 ${color}`} />
                </div>
                <div>
                  <p className='text-sm font-extrabold text-gray-900 dark:text-[#F0EFEA]'>
                    {value}
                  </p>
                ...
                {[
                  ...
                  <div key={label} className='flex items-start gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm font-semibold text-gray-900 dark:text-[#F0EFEA]'>
                        {value}
                      </p>
                    </div>
                  </div>
              ...
              >
                ...
                <div className='p-4 space-y-2.5'>
                  {[
                    {
                      label: 'Ex-SM ID Card',
                      uploaded: !!profile.id_card_path,
                    },
                    {
                      label: 'Discharge Book',
                      uploaded: !!profile.discharge_book_path,
                    },
                    {
                      label: 'Police Verification Cert.',
                      uploaded: !!profile.police_verification_path,
                    },
                  ].map(({ label, uploaded }) => (
                    <div key={label} className='flex items-center gap-2.5'>
                      ...
                      <span
                        className={`text-xs ${uploaded ? 'text-gray-700 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500'}`}
                      >
                        {label}
                      </span>
                    </div>
                ...
                <div className='p-4 space-y-3'>
                  {[
                    ...
                    <div key={n} className='flex items-center gap-2.5'>
                      ...
                      <span
                        className={`text-xs font-medium ${done ? 'text-gray-900 dark:text-[#F0EFEA]' : 'text-gray-400 dark:text-gray-500'}`}
                      >
                        {label}
                      </span>
                    </div>
          ...
          >
            ...
            <div className='py-12 text-center'>
              ...
              <p className='text-xs text-gray-400 dark:text-gray-500 max-w-xs mx-auto'>
                Once your profile is verified, Ex-Serviceman Jobs will match you
                with relevant employers and notify you here.
              </p>
            </div>
          </motion.div>
        </>
      ...
      {!loading && !error && !profile && (
        ...
        >
          ...
          <p className='text-sm font-semibold text-gray-900 dark:text-[#F0EFEA] mb-1'>
            Complete your registration
          </p>
          <p className='text-xs text-gray-500 dark:text-gray-400 mb-4'>
            Fill in your profile details to get matched with employers.
          </p>
          <button
