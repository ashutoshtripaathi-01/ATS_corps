...
function getNotifColor(type: string): string {
  switch (type) {
    case 'job-alert':
      return 'bg-[#F7A607]/15 text-[#F7A607]';
    case 'application':
...
function NotifItem({ notif }: { notif: Notification }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 cursor-pointer border-b border-gray-50 dark:border-white/5 last:border-0 transition-colors hover:bg-gray-50 dark:hover:bg-white/5 ${
        !notif.read ? 'bg-[#F7A607]/5 dark:bg-[#F7A607]/8' : ''
      }`}
    >
      <div
        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${getNotifColor(notif.type)}`}
      >
        <Icon className='w-4 h-4' />
      </div>
      <div className='flex-1 min-w-0'>
        <p
          className={`text-sm leading-snug mb-0.5 ${!notif.read ? 'font-semibold text-gray-900 dark:text-[#F0EFEA]' : 'font-medium text-gray-700 dark:text-gray-200'}`}
        >
          {notif.title}
        </p>
        ...
        <p className='text-[11px] text-[#F7A607] font-semibold mt-1'>
          {timeAgo(new Date(notif.createdAt))}
        </p>
      </div>
      {!notif.read && (
        <div className='w-2.5 h-2.5 rounded-full bg-[#F7A607] shrink-0 mt-1.5' />
      )}
    </div>
  );
}
...
const SEED_NOTIFICATIONS: Notification[] = [
  {
    id: 'seed-1',
    type: 'job-alert',
    title: 'New job match: Security Guard',
    body: 'A new position matching your profile is open in Guwahati · ₹18k/month, night shift available.',
    read: false,
  ...
  },
  {
    ...
    body: 'Your application for Armed Guard at Assam Security Pvt Ltd is currently being reviewed by the employer.',
    read: false,
  ...
  },
  {
    id: 'seed-3',
    type: 'system',
    title: 'Welcome to Ex-Serviceman Jobs!',
    body: 'Your account has been verified. Complete your profile to get hired faster by top security companies.',
    read: false,
            ...
            <div className='flex flex-col items-center gap-2'>
              ...
              <div className='w-full'>
                <div className='flex justify-between text-xs mb-1'>
                  ...
                  <span className='text-[#F7A607] font-semibold'>
                    {profileStrength}%
                  </span>
                </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all relative ${
                  active
                    ? 'bg-[#F7A607] text-white shadow-lg shadow-[#F7A607]/25'
      ...
      <div className='flex-1 flex flex-col overflow-hidden min-w-0'>
        ...
        <header className='h-16 bg-white dark:bg-[#1e2227] border-b border-gray-100 dark:border-white/8 flex items-center gap-3 px-3 sm:px-5 sticky top-0 z-40 shadow-sm dark:shadow-black/20'>
          ...
          <Link to='/' className='md:hidden flex items-center gap-2 shrink-0'>
            ...
            <span className='font-bold text-base text-gray-900 dark:text-[#F0EFEA]'>
              {BRAND.name}
            </span>
          </Link>
          ...
          <div className='ml-auto flex items-center gap-2'>
            ...
            <div className='relative' ref={notifRef}>
              ...
              >
                <Bell
                  className={`w-5 h-5 transition-colors ${notifOpen ? 'text-[#F7A607]' : 'text-gray-600 dark:text-gray-300'}`}
                />
                {unreadCount > 0 && (
                ...
                {notifOpen && (
                  ...
                  >
                    <div className='flex items-center justify-between px-4 py-3.5 border-b border-gray-100 dark:border-white/8'>
                      <h3
                        className='text-base font-extrabold text-gray-900 dark:text-[#F0EFEA]'
                        style={{ fontFamily: 'Plus Jakarta Sans' }}
                      >
                        Notifications
                      </h3>
                      {unreadCount > 0 && (
                        <button
                          onClick={markAllRead}
                          className='text-xs font-semibold text-[#F7A607] hover:text-[#d99400] transition-colors'
                        >
                          Mark all as read
                        </button>
                      )}
                    ...
                    <div className='max-h-[420px] overflow-y-auto divide-y divide-gray-50 dark:divide-white/5'>
                      {notifications.length === 0 ? (
                        <div className='flex flex-col items-center justify-center py-12 px-4'>
                          ...
                          <p className='text-sm font-semibold text-gray-700 dark:text-[#F0EFEA]'>
                            No notifications yet
                          </p>
                          <p className='text-xs text-gray-400 dark:text-gray-500 text-center mt-1'>
                            Job alerts and updates will appear here
                          </p>
                        </div>
                      ) : (
                        notifications.map((n) => (
            ...
            >
              <div className='flex items-center justify-between p-4 border-b border-white/10'>
                <div className='flex items-center gap-2'>
                  <img src={companyLogo} alt='Ex-Serviceman Jobs' className='w-8 h-8 object-contain' />
