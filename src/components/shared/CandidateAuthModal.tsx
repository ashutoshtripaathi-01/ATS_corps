...
import { z } from 'zod'
import {
  ArrowLeft, Phone, Shield, CheckCircle, Eye, EyeOff,
  User, MapPin, Briefcase, ChevronRight, RefreshCw, Zap,
} from 'lucide-react'
...
export function CandidateAuthModal({ open, onClose, onBack, mode: initialMode }: CandidateAuthModalProps) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast({ title: 'OTP Resent', description: `New OTP sent to +91 ${mobile}.` })
    } catch (err: any) {
      toast({ title: 'Failed to resend OTP', description: err.message || 'Please try again.', variant: 'error' })
    }
  }
  ...
  const experienceLevels = ['Fresher', '0–1 yr', '1–3 yrs', '3–5 yrs', '5–10 yrs', '10+ yrs']
        ...
        <div className="bg-white">
          ...
          <div className="px-5 pt-4 pb-2">
            <div className="flex items-center gap-2">
              {(mode === 'register' ? ['Mobile', 'OTP', 'Details'] : ['Mobile', 'OTP']).map((s, i) => {
                const stepIndex = ['mobile', 'otp', 'profile', 'success'].indexOf(step)
                ...
                return (
                  <div key={s} className="flex items-center gap-2 flex-1">
                    ...
                    {i < (mode === 'register' ? 2 : 1) && <div className={`flex-1 h-0.5 rounded-full ${isDone ? 'bg-green-400' : 'bg-gray-100'}`} />}
                  </div>
                )
              })}
            </div>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <form onSubmit={mobileForm.handleSubmit(handleSendOtp)} className="space-y-4">
                  <div>
                    ...
                    <Button type="submit" size="lg" className="w-full gap-2" disabled={sendingOtp}>
                    {sendingOtp ? (
                ...
                <button
                  ...
                  onClick={() => {
                    onClose()
                    if (mode === 'register') {
                      navigate('/candidate/register', { state: { mobile: '9999999999' } })
                    } else {
                ...
                {mode === 'login' && (
                  <p className="text-center text-xs text-gray-500 mt-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setMode('register'); setStep('mobile'); mobileForm.reset() }}
                ...
                {mode === 'register' && (
                  <p className="text-center text-xs text-gray-500 mt-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setMode('login'); setStep('mobile'); mobileForm.reset() }}
                      className="text-[#F7A607] font-semibold hover:underline"
                    >
                      Login here
                    </button>
            ...
            {step === 'otp' && (
              ...
              >
                ...
                <Button size="lg" className="w-full gap-2 mb-4" onClick={handleVerifyOtp} disabled={verifying || otp.join('').length < 6}>
                  {verifying ? (
                ...
                <div className="flex items-center justify-center gap-1.5">
                  {canResend ? (
                    <button onClick={handleResend} className="text-sm text-[#F7A607] font-semibold hover:underline flex items-center gap-1">
            ...
            {step === 'profile' && (
              <motion.div
                ...
                className="px-6 pt-4 pb-7"
              >
                <h3 className="text-xl font-extrabold text-[#242424] mb-1" style={{ fontFamily: 'Plus Jakarta Sans' }}>
                  Complete Your Profile
                </h3>
                <p className="text-sm text-gray-500 mb-5">Just a few details to get started</p>
                ...
                <form onSubmit={profileForm.handleSubmit(handleProfileSubmit)} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Harshit Sharma" {...profileForm.register('name')} className="pl-9" autoFocus />
                    </div>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-1 block">Current City *</label>
                    <div className="relative">
                      <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <Input placeholder="Bangalore" {...profileForm.register('location')} className="pl-9" />
                    </div>
                    {profileForm.formState.errors.location && (
                      <p className="text-xs text-red-500 mt-1">{profileForm.formState.errors.location.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Frontend Developer" {...profileForm.register('jobTitle')} className="pl-9" />
                    </div>
                    {profileForm.formState.errors.jobTitle && (
                      <p className="text-xs text-red-500 mt-1">{profileForm.formState.errors.jobTitle.message}</p>
                    )}
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-gray-700 mb-2 block">Experience *</label>
                    <div className="flex flex-wrap gap-2">
                      {experienceLevels.map((level) => {
                        const selected = profileForm.watch('experience') === level
                        return (
                          <button
                            key={level}
                            type="button"
                            onClick={() => profileForm.setValue('experience', level, { shouldValidate: true })}
                    ...
                    {profileForm.formState.errors.experience && (
                      <p className="text-xs text-red-500 mt-1">{profileForm.formState.errors.experience.message}</p>
                    )}
                  </div>

                  <Button type="submit" size="lg" className="w-full gap-2 mt-2" disabled={profileForm.formState.isSubmitting}>
                    {profileForm.formState.isSubmitting ? (
                      <span className="flex items-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" /> Creating Account...</span>
                    ) : (
                      <span className="flex items-center gap-2">Start Job Hunting! 🚀</span>
                    )}
                  </Button>
                </form>
              </motion.div>
            ...
            {step === 'success' && (
              ...
              >
                ...
                <p className="text-gray-500 text-sm mb-2">
                  {mode === 'login' ? 'Signed in successfully' : 'Your candidate account is ready'}
                </p>
